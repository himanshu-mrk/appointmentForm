...
import checkDuplicateAppointment from '@salesforce/apex/AppointmentFormHandler.checkDuplicateAppointment';
...
export default class AppointmentForm extends LightningElement {
    ...
    handleSave() { //Validates the data from UI and if data is valid using server call save the appointment details
        ...
        if (this.isSlotBooked) {
            this.showErrorToast('The selected slot is already booked please select another slot');
            return;
        }
    ...
    checkDuplicasy() { //check is the slot already booked for selected time slot
        checkDuplicateAppointment({ appointmentDateTime: this.appointmentDateTime })
            .then(result => {
                if (result) {
